                    ...
                    <tbody>
                        @foreach ($reports as $index => $report)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    @if ($report->contract_group)
                                        <button class="btn btn-sm btn-danger btn-export-contract"
                                            data-contacts-id="{{ $report->contract_group->id }}">
                                            <i class="fa fa-file-excel"></i> EXPORT
                                        </button>

                                        <button class="btn btn-sm btn-primary btn-view-contract"
                                            data-id="{{ $report->contract_group->id }}" data-from="{{ request('from') }}"
                                            data-to="{{ request('to') }}">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            $(document).ready(function() {
                ...
                $('#FilterButtonReport').on('click', function() {
                    ...
                    window.location.href = `/reports/report/all?from=${from}&to=${to}`;
                });

                $('#refresh').on('click', function() {
                    window.location.href = `/reports/report/all`;
                });
                ...
                $('#typeContractTable tbody').on('click', '.btn-view-contract', function() {
                    ...
                    if (row.child.isShown()) {
                        ...
                        $.ajax({
                            url: `/reports/details/all/${reportId}?from=${from}&to=${to}`,
                            method: 'GET',
                            data: {
                                from: from,
                                to: to
                                ...
                                $('#details-' + reportId).html(data.map((item, index) => `
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <button 
                                                class="btn btn-sm btn-primary btn-detail-toko-all" 
                ...
                function format(id) {
                 ...
                 // Klik tombol detail
                 $(document).on('click', '.btn-detail-toko-all', function() {
                    ...
                    let url =
                        `/reports/details/all/${contactsId}/${regionId}?from=${from}&to=${to}`;
                ...
                $(document).on('click', '.btn-export-contract', async function(e) {
                    ...
                    try {
                        ...
                        const response = await fetch(
                            `/reports/details/export/all/${contractId}/contract?from=${from}&to=${to}`);
