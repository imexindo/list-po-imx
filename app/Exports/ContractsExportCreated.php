...
class ContractsExportCreated implements FromCollection, WithHeadings, WithMapping, WithStyles
{
    protected $from;
    protected $to;
    protected $region_id;
    ...
    public function __construct($from, $to, $region_id)
    {
        $this->from = $from;
        $this->to = $to;
    ...
    public function collection()
    {
        return Contracts::select([
            't_contracts.*',
            'm_contract.code as contract_code'
        ])
            ->with(['category', 'contract', 'type_contracts'])
            ->when($this->from && $this->to, function ($query) {
                $query->whereBetween('t_contracts.updated_at', [
                    $this->from . ' 00:00:00',
                    $this->to . ' 23:59:59'
                ]);
            })
            ->when($this->region_id, function ($query) {
                $query->where('t_contracts.region_id', $this->region_id);
            })
            ->join('m_contract', 't_contracts.contract_id', '=', 'm_contract.id')
            ->orderBy('t_contracts.tgl_bap')
            ->orderBy('t_contracts.group_contract_id')
            ->orderBy('t_contracts.region_id')
            ->orderBy('m_contract.code')
            ->get();
    }
